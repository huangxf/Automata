...
public class epsnfa{
	...
	int[] union(int s1,int t1,int s2,int t2)
	{
        ...
        int start = incCapacity();
        int end = incCapacity();

        addEdge(start, symbol, s1);
        addEdge(t1, symbol, end);

        addEdge(start, symbol, s2);
        addEdge(t2,symbol, end);
	...
	int[] clo(int s,int t)
	{
        ...
        addEdge(t, symbol, s);
        addEdge(start, symbol, s);
        addEdge(t, symbol, end);
        addEdge(start, symbol, end);
	...
	public static void main(String args[]) {
		new epsnfa().Start("D:\\Automata\\Convertion\\Resource\\SampleRE.in");
	}
}
